...
def get_plate(wpod_net_model, image, Dmax=700, Dmin=500):
    ratio = float(max(image.shape[:2])) / min(image.shape[:2])  # vehicle.shape[:2] = (720,1280), ratio = 1.77777778
    side = int(ratio * Dmin)
    bound_dim = min(side, Dmax)
    _, LpImg, LpType, cor = detect_lp(wpod_net_model, image, bound_dim, lp_threshold=0.5)
    plate_image = cv2.convertScaleAbs(LpImg[0], alpha=255.0)
    return plate_image, LpType, cor